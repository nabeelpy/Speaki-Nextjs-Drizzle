...
export default function QuickStartCoursePage({
  ...
  if (isLoading || !course) {
    return (
      <div className="min-h-screen bg-[#f6f7f8] dark:bg-slate-900">
        <Header />
    ...
    <div className="relative flex h-auto min-h-screen w-full flex-col group/design-root overflow-x-hidden">
      <div className="layout-container flex h-full grow flex-col">
        ...
        <main className="flex flex-col w-full max-w-[1200px] mx-auto px-4 lg:px-10 py-10 pb-24 md:pb-10">
          ...
          <div className="bg-gradient-to-r from-[#137fec]/20 to-[#137fec]/5 rounded-2xl p-8 mb-8">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <h1 className="text-4xl font-bold text-[#0d141b] dark:text-white mb-2">
                  {course.title}
                </h1>
                <p className="text-[#4c739a] dark:text-slate-400">{course.category}</p>
              </div>
            </div>
            <p className="text-lg text-[#4c739a] dark:text-slate-400 mb-6">{course.description}</p>
            <div className="flex flex-wrap gap-6">
              <div>
                <p className="text-[#4c739a] dark:text-slate-400 text-sm">Instructor</p>
                <p className="font-bold text-[#0d141b] dark:text-white">{course.instructor}</p>
              </div>
              <div>
                <p className="text-[#4c739a] dark:text-slate-400 text-sm">Duration</p>
                <p className="font-bold text-[#0d141b] dark:text-white">{course.duration} minutes</p>
              </div>
              <div>
                <p className="text-[#4c739a] dark:text-slate-400 text-sm">Lessons</p>
                <p className="font-bold text-[#0d141b] dark:text-white">{course.lessons.length} lessons</p>
              </div>
              <div>
                <p className="text-[#4c739a] dark:text-slate-400 text-sm">Rating</p>
                <p className="font-bold text-[#0d141b] dark:text-white">
                  {course.rating.toFixed(1)} ★ ({course.ratingCount} reviews)
                </p>
              </div>
              <div>
                <p className="text-[#4c739a] dark:text-slate-400 text-sm">XP Reward</p>
          ...
          {/* Lessons Section */}
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-[#0d141b] dark:text-white mb-6">Course Content</h2>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-[#4c739a] dark:text-slate-400">{lesson.duration} minutes</p>
                    </div>
                  </div>
                  <Link
                    href={`/quick-start/lessons/${(lesson as { conversationId?: string }).conversationId ?? `l${index + 1}`}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white dark:bg-slate-900 border border-[#e7edf3] dark:border-slate-800 rounded-lg p-6">
              <h3 className="text-[#4c739a] dark:text-slate-400 text-sm font-bold mb-2">Enrolled Students</h3>
              <p className="text-2xl font-bold text-[#0d141b] dark:text-white">
                {course.enrollmentCount.toLocaleString()}
              </p>
            </div>
            <div className="bg-white dark:bg-slate-900 border border-[#e7edf3] dark:border-slate-800 rounded-lg p-6">
              <h3 className="text-[#4c739a] dark:text-slate-400 text-sm font-bold mb-2">Average Rating</h3>
              <p className="text-2xl font-bold text-[#0d141b] dark:text-white">
            ...
            <div className="bg-white dark:bg-slate-900 border border-[#e7edf3] dark:border-slate-800 rounded-lg p-6">
              <h3 className="text-[#4c739a] dark:text-slate-400 text-sm font-bold mb-2">XP Reward</h3>
