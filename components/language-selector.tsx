...
const LANGUAGES = [
    {
        ...
        name: 'English',
        flag: '🇬🇧',
        voiceKeywords: ['english', 'en-US', 'en-GB', 'en-AU', 'daniel', 'samantha', 'karen']
    },
    {
        code: 'fr',
        name: 'French',
        flag: '🇫🇷',
        voiceKeywords: ['french', 'fr-FR', 'fr-CA', 'thomas', 'amélie', 'amelie']
    },
    {
        code: 'es',
        name: 'Spanish',
        flag: '🇪🇸',
        voiceKeywords: ['spanish', 'es-ES', 'es-US', 'es-MX', 'mónica', 'monica', 'jorge']
    },
    {
        code: "zh",
        name: "Chinese",
...
function StatusBadge({status}: { status: 'installed' | 'missing' | 'unknown' }) {
    if (status === 'installed') {
        return (
            <span
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-emerald-500/15 text-emerald-600 border border-emerald-500/30">
    ...
    if (status === 'missing') {
        return (
            <span
                className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-rose-500/15 text-rose-600 border border-rose-500/30">
        <XCircle size={10}/>
        Setup
      </span>
        )
    ...
    return (
        <span
            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium bg-amber-500/15 text-amber-600 border border-amber-500/30">
      <AlertCircle size={10}/>
      Checking
    </span>
    )
...
${status !== 'installed' ? 'min-w-[33%] sm:min-w-[50%] md:min-w-[200px]' : 'min-w-[26%] md:min-w-0'}
              ...
              `}
                        >
                            ...
                            {isSelected && (
                                <div
                                    className="absolute top-2 right-2 w-4 h-4 rounded-full bg-[#137fec] flex items-center justify-center">
                            ...
                            {status !== 'missing' && (
                                <span
                                    className={`mb-auto text-xs font-semibold md:text-sm ${
